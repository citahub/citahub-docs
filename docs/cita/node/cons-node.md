...
CITA 采用[共识节点管理系统合约]来实现共识节点的管理，通过区块链上的合约可以保证共识节点的安全性及一致性。
...
* 发送交易
  ...
  其中 `--admin-privkey` 是管理员私钥，系统默认的管理员私钥可以看 [管理员账户信息]。
...
我们提供 [setStake 接口] 可对共识节点出块权重进行配置。

[共识节点管理系统合约]: ../system-contract-interface/node
[setStake 接口]: https://docs.citahub.com/zh-CN/0.23.0/cita/system-contract-interface/node#setstake
[solc]: https://solidity-cn.readthedocs.io/zh/develop/installing-solidity.html
[管理员账户信息]: ../getting-started/run-cita
