...
class SideNav extends React.Component {
  ...
  // renderSubcategory hacked by wu 2019-06-10
  renderSubcategory = subcategoryItem => {
    let ulClassName = '';
    let subCategoryClassName = 'navGroupSubcategoryTitle';
    let arrow;

    if (siteConfig.docsSideNavCollapsible) {
      subCategoryClassName += ' collapsible';
      ulClassName = 'hide';
      arrow = (
        <span className="arrow">
          <svg width="24" height="24" viewBox="0 0 24 24">
    ...
    return (
      <div className="navGroup subNavGroup" key={subcategoryItem.title}>
        <h4 className={subCategoryClassName}>
          {this.getLocalizedCategoryString(subcategoryItem.title)}
          {arrow}
        </h4>
        <ul className={ulClassName}>
          {subcategoryItem.children.map(item => {
            switch (item.type) {
              case 'LINK':
                return this.renderItemLink(item);
              case 'SUBCATEGORY':
