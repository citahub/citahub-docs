...
存证是向存证指令地址发交易，要存证的内容放在交易的data字段。读取是根据存证交易哈希调用 getTransaction，然后将结果中的 content 反序列化，得到交易的信息，其中的 data 字段即为存证内容。详见[存证操作说明](./store.md)。
...
做快照是可以减少目录大小，但老的数据会丢失。 日志可以清理掉或者备份到其他服务器上，参考 CITA 文档：https://docs.citahub.com/zh-CN/cita/ops/log
