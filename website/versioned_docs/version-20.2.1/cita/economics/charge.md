---
id: version-20.2.1-charge
title: Charge 模式
original_id: charge
---

Charge 模式，链上是存在原生代币的，运营方不仅可以对原生代币自身的一些属性如名字，标识等，请参考 [链级配置] 进行配置，也可以对代币总量，代币的分发方式，代币的流转模型进行个性化定制。

代币在链上是以最小单位 `wei` 计量的，比如 `getBalance` 查询到的账户的余额就是以 `wei` 为单位的数值。

在浏览器和多链客户端中，为了显示上的方便，会采用一个更大的单位 `CTT` 。这个单位的名字可以由链的运营方自定义，`CTT` 是默认的名字。两个单位之间的换算关系为 `1 CTT = 10^18 wei`。

注意：后续文档中，除非特殊说明，代币默认采用的单位都是 `wei` 。
...
设置链的代币总量为 10_000_000_000 CTT，即 10^28 (0x204fce5e3e25026110000000) wei：

```shell
$ bin/cita create --init_token 0x204fce5e3e25026110000000 --contract_arguments SysConfig.economicalModel=1 --super_admin "0x4b5ae4567ad5d9fb92bc9afd6a657e6fa13a2523" --nodes "127.0.0.1:4000,127.0.0.1:4001,127.0.0.1:4002,127.0.0.1:4003"
```
...
* 超级管理员可以通过发交易的方式进行分配；
* 也可以使用我们提供的[水龙头工具](https://github.com/citahub/cita-testnet-faucet)，让用户自己领取代币；
...
运营方可以通过调节 Quota Price 高低来调整原生代币的使用价值。
