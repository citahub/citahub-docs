...
* `address`: `Data20 | [Data20]` - **可选** 合约地址或者地址的集合
* `topics`: `[Data32 | [Data32]]` - **可选** 日志 `topic` 或者 `topic` 集合

`Topics` 是顺序相关的, 过滤器通过以下规则来根据 `topic` 过滤交易日志:

* `[]`: 不设置任何 `topic`
* `[A]`: `topics`第一个索引是 `A`, 其余 `topic` 无限制
* `[null, B]`: `topics` 第一个索引没有限制, 第二个索引是 `B`, 其余 `topics` 无限制
* `[A, B]`: `topics` 第一个索引是 `A`, 第二个索引是 `B`, 其余 `topics` 无限制
* `[[A, B], [A, B]]`: `topics` 第一个索引是 `A` 或者 `B`, 第二个索引是 `A` 或者 `B`, 其余 `topics` 无限制
