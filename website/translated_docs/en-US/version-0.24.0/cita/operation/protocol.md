...
CITA 同样采用类似公链分叉的方式进行协议升级, 只不过我们的升级方案是通过增加一个专门的 `version_manager` 合约，存放一个单调递增的协议版本号, 超级管理员通过发送交易变更版本号来触发协议升级。 节点数较少的情况下，相对可控，要求所有节点先统一实施 `软件升级`，然后再实施 `协议升级`。
